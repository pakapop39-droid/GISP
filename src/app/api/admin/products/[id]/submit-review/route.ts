...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { data, error } = await insforge.database.rpc("submit_product_for_review", { product_id_input: id });
    if (error) throw error;
    return NextResponse.json({ data: { id: data }, message: "ส่ง Product เข้าตรวจแล้ว" });
