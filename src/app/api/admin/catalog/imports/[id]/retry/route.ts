...
export async function POST(_request: NextRequest,{params}:{params:Promise<{id:string}>}) {
  ...
  try{await requireAppAccess({permissions:["catalog.import"]});if(!isPdfCatalogImportEnabled())return NextResponse.json({code:"FEATURE_DISABLED",message:"PDF Catalog Import ยังไม่ได้เปิดใน Environment นี้"},{status:404});const db=await createInsForgeServerClient();const result=await db.database.rpc("retry_catalog_pdf_import",{import_job_id_input:id});if(result.error)throw result.error;return NextResponse.json({data:result.data,message:"นำหน้าที่ล้มเหลวกลับเข้าคิวแล้ว"});}catch(error){return apiError(error);}
}
