...
import { CatalogImportHistoryButton, revealCatalogImportDetail, type CatalogImportHistoryJob } from "@/components/catalog-import-history";
...
export function CatalogImportWorkspace({ canImport, canManage, canReadCosts=false, canManageCosts=false, pdfEnabled = false, excelRoundtripEnabled=false }: { canImport: boolean; canManage: boolean; canReadCosts?:boolean; canManageCosts?:boolean; pdfEnabled?: boolean; excelRoundtripEnabled?:boolean }) {
  ...
  return <div className="space-y-5">
    ...
    {detail ? <div ref={detailSectionRef} tabIndex={-1} aria-labelledby="catalog-import-current-job" className="space-y-4 scroll-mt-4 outline-none focus-visible:ring-2 focus-visible:ring-[#356b52]/50">
      <section className="rounded-xl border border-[#356b52]/30 bg-[#356b52]/5 px-4 py-3"><div className="flex flex-wrap items-center justify-between gap-2"><h2 id="catalog-import-current-job" className="font-serif text-xl">กำลังดูงาน: {detail.job.file_name}</h2><span className="v14-status">{detail.job.status}</span></div></section>
