...
export const adminSourcingStatuses = sourcingStatuses.filter(status=>status!=="DRAFT");
export function isAdminVisibleSourcingStatus(status:string){return status!=="DRAFT"&&sourcingStatuses.includes(status as SourcingStatus)}
