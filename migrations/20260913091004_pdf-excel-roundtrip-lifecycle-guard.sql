...
BEGIN
  target_batch:=CASE WHEN TG_TABLE_NAME='catalog_import_enrichment_batches' THEN NEW.id ELSE NEW.batch_id END;
  ...
  IF job_status NOT IN ('READY_FOR_REVIEW','COMPLETED','COMPLETED_WITH_ISSUES') THEN
    IF TG_OP='INSERT' OR TG_TABLE_NAME='catalog_import_enrichment_batches' AND NEW.status<>'CANCELLED' OR
       TG_TABLE_NAME='catalog_import_enrichment_rows' AND
...
CREATE TRIGGER catalog_import_enrichment_batches_lifecycle
BEFORE INSERT OR UPDATE ON public.catalog_import_enrichment_batches
FOR EACH ROW EXECUTE FUNCTION public.guard_catalog_enrichment_lifecycle();
CREATE TRIGGER catalog_import_enrichment_rows_lifecycle
BEFORE INSERT OR UPDATE ON public.catalog_import_enrichment_rows
FOR EACH ROW EXECUTE FUNCTION public.guard_catalog_enrichment_lifecycle();
