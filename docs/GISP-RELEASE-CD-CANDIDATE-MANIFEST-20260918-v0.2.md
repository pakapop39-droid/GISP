# GISP Release C/D Candidate Manifest v0.2 — local source freeze only

Supersedes v0.1 for inventory/hash review; v0.1 remains unchanged. Owner references: `GISP-APR-RELEASE-CD-IMPLEMENTATION-20260918-v0.1.md` and `GISP-APR-PAYMENT-EVIDENCE-BINDING-20260918-v0.1.md`. This is **not** authority to Apply a Migration, Deploy, mutate Production, or accept real payments. The dirty working tree includes unrelated work; only the 36 enumerated file snapshots are this candidate, and even shared files require hunk-level review before packaging. SHA-256 values below were read from the local files on 2026-09-18 (Asia/Bangkok); do not package the whole worktree.

QA disposition at this local candidate stage: **PASS WITH CONDITIONS** for code and automated checks; **Production FAIL**. No Payment migration has been applied or transaction-retested, no complete Production rehearsal or Release C/D authorization exists, and the database alone cannot inspect stored file bytes. Finance must still open the exact stored file through the App and compare it with the Payment before approval. The one-file/one-Customer-Payment-Transfer policy is permanent, including rejected transfers; a new Payment needs a newly uploaded file. Legacy unbound files cannot be approved automatically.

## Application code and automated tests (28 files)
...
| `e6eb3c8b0c68405aaa97e6c9baf23bc9d7d47fd543cb9f8f22e975bd40282628` | `src/lib/payments/bound-evidence.ts` — v0.2 exact `entity_id` binding |
| `20ecd976553ec5215c10576b62ee4d17a762ecfce3e7f720c71b50b11a02974b` | `src/lib/payments/bound-evidence.test.ts` — v0.2 legacy-null rejection |
...
## Permission and Payment migration drafts (6 files; none applied)

Run only after fresh target-environment function-signature/ACL audit, rehearsal, QA review, backup/rollback verification, and separate owner release approval. C precedes D7, D8, D9, D10; each D Slice has its own gate. Production B lacks `reopen_qc_inspection`, so the separately approved QC function migration is a D7 prerequisite, not an automatic member of this inventory. Payment evidence binding is a **C prerequisite** and must be reviewed against the consolidated target baseline before any Development/rehearsal Apply. Its filename sorts after the D drafts, so a generic “apply all pending migrations” would open later permissions too early. **Do not use migrate-all**; release sequencing and an explicitly selected migration set must be resolved and approved before rehearsal or Production.
...
| Payment binding | `5a8cc2a7308fd78a7dba87244871dae08d30e1665b2193694e9b9e97eb94dac3` | `migrations/20260918211000_payment-evidence-binding.sql` |

## New Payment test and approval record (2 files)

| SHA-256 | Path |
|---|---|
| `74492a4fd1d2059e3cfa600780f971dece332a8eddb3f7d12c9c3e1b5c75f8a8` | `src/lib/payments/evidence-binding-migration.test.ts` |
| `2055d371be8d4e572068e968bbbb4029a25c5d87db934f5c2247a3f8675cdbbb` | `docs/GISP-APR-PAYMENT-EVIDENCE-BINDING-20260918-v0.1.md` |

Runtime configuration remains separate from this file set: `RELEASE_STAGE=C` for C; D requires `RELEASE_STAGE=D` and a contiguous `RELEASE_D_ENABLED_SLICES` list (`7`, `7,8`, `7,8,9`, or `7,8,9,10`). No `.env`, key, credential, branch operation, data migration, or Production setting is included. Excluded from automatic apply/deploy: all other dirty worktree files and standalone Slice 4–11, ISS-02, QC-Reopen, and unrelated migrations. The global `files.member.manage` model remains a QA observation, not a confirmed cross-organization exploit.
