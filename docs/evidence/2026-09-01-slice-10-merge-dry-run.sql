-- Generated 2026-08-31T23:02:03.781Z
-- ⚠️ MERGE BLOCKED: 1 conflict(s) detected. Resolve before applying.

-- [CONFLICT] config_row schedules.jobs
--   parent_t0_hash:  41a2faa239b3c7c99e64cecab2c5013b63eace55f4e9cdfd6c99b2c39b264055
--   parent_now_hash: 43881fb5b9f3f3fa5321cb664e1114d8b450297c2facce44e91d429cd3293f82
--   branch_now_hash: 55ed6d67d7c07c701d749820bd18a610500c734514fd45f981839d4407b1e065
...
-- ===== MIGRATION =====
-- [MIGRATION] migration system.20260831145347 (add)
-- Migration 20260831145347: slice-10-dashboard-reports
...
GRANT EXECUTE ON FUNCTION public.record_fixed_report_export(TEXT, JSONB, INTEGER)
  TO authenticated;
INSERT INTO "system"."custom_migrations" ("version", "name", "statements", "created_at") VALUES ('20260831145347', 'slice-10-dashboard-reports', ARRAY['INSERT INTO public.permissions(code, name)
VALUES
  (''reports.fixed.read'', ''ดู Fixed Report''),
  (''reports.fixed.export'', ''Export Fixed Report'')
ON CONFLICT (code) DO UPDATE SET name = EXCLUDED.name', 'INSERT INTO public.role_permissions(role_id, permission_id)
SELECT r.id, p.id
FROM public.roles r
JOIN public.permissions p ON p.code IN (''reports.fixed.read'', ''reports.fixed.export'')
WHERE r.code IN (
...
ON CONFLICT DO NOTHING', 'CREATE INDEX IF NOT EXISTS customer_orders_member_report_idx
  ON public.customer_orders(member_profile_id, created_at DESC, status)', 'CREATE INDEX IF NOT EXISTS payment_schedules_report_idx
  ON public.payment_schedules(order_id, created_at DESC, status, due_at)', 'CREATE INDEX IF NOT EXISTS production_updates_latest_report_idx
  ON public.production_updates(supplier_order_id, created_at DESC)', 'CREATE INDEX IF NOT EXISTS shipments_report_idx
  ON public.shipments(customer_order_id, created_at DESC, status)', 'CREATE INDEX IF NOT EXISTS shipment_status_delay_report_idx
  ON public.shipment_status_history(shipment_id, event_at DESC)
  WHERE is_delay = TRUE', 'CREATE INDEX IF NOT EXISTS deliveries_report_idx
  ON public.deliveries(customer_order_id, created_at DESC, status, scheduled_at)', 'CREATE OR REPLACE FUNCTION public.get_member_dashboard()
RETURNS JSONB
LANGUAGE PLPGSQL
STABLE
SECURITY DEFINER
...
END;
$$', 'REVOKE ALL ON FUNCTION public.get_member_dashboard()
  FROM PUBLIC, anon, authenticated', 'REVOKE ALL ON FUNCTION public.get_admin_operations_dashboard()
  FROM PUBLIC, anon, authenticated', 'REVOKE ALL ON FUNCTION public.get_executive_dashboard(DATE, DATE)
  FROM PUBLIC, anon, authenticated', 'REVOKE ALL ON FUNCTION public.get_fixed_report(TEXT, DATE, DATE, TEXT, INTEGER, INTEGER)
  FROM PUBLIC, anon, authenticated', 'REVOKE ALL ON FUNCTION public.record_fixed_report_export(TEXT, JSONB, INTEGER)
  FROM PUBLIC, anon, authenticated', 'GRANT EXECUTE ON FUNCTION public.get_member_dashboard()
  TO authenticated', 'GRANT EXECUTE ON FUNCTION public.get_admin_operations_dashboard()
  TO authenticated', 'GRANT EXECUTE ON FUNCTION public.get_executive_dashboard(DATE, DATE)
  TO authenticated', 'GRANT EXECUTE ON FUNCTION public.get_fixed_report(TEXT, DATE, DATE, TEXT, INTEGER, INTEGER)
  TO authenticated', 'GRANT EXECUTE ON FUNCTION public.record_fixed_report_export(TEXT, JSONB, INTEGER)
  TO authenticated'], '2026-08-31T14:57:08.330572+00:00')
  ON CONFLICT ("version") DO UPDATE SET "name" = EXCLUDED."name", "statements" = EXCLUDED."statements", "created_at" = EXCLUDED."created_at";

-- ===== DDL =====
-- [DDL] function public.get_member_dashboard() (add)
CREATE OR REPLACE FUNCTION public.get_member_dashboard()
 RETURNS jsonb
 LANGUAGE plpgsql
...
-- [DDL] function public.get_admin_operations_dashboard() (add)
CREATE OR REPLACE FUNCTION public.get_admin_operations_dashboard()
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
...
-- [DDL] function public.get_executive_dashboard(date_from_input date, date_to_input date) (add)
CREATE OR REPLACE FUNCTION public.get_executive_dashboard(date_from_input date, date_to_input date)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'pg_temp'
...
-- [DDL] function public.record_fixed_report_export(report_type_input text, filters_input jsonb, row_count_input integer) (add)
CREATE OR REPLACE FUNCTION public.record_fixed_report_export(report_type_input text, filters_input jsonb, row_count_input integer)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'pg_temp'
...
-- [DDL] function public.get_fixed_report(report_type_input text, date_from_input date, date_to_input date, status_filter_input text, row_limit_input integer, row_offset_input integer) (add)
CREATE OR REPLACE FUNCTION public.get_fixed_report(report_type_input text, date_from_input date, date_to_input date, status_filter_input text DEFAULT NULL::text, row_limit_input integer DEFAULT 500, row_offset_input integer DEFAULT 0)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'pg_temp'
