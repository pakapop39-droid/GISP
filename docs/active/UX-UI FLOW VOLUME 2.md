...
**Change Summary:** เพิ่ม Price/Warranty Snapshot, Factory Disclosure และ Claim Responsibility ตาม DEC-037 ถึง DEC-042  
...
* Suggested Resale/Freight Estimate เป็นข้อมูลแนะนำจาก Project Snapshot ไม่ใช้รวมยอด Order/Payment
* Order Item เก็บ Partner Warranty Snapshot และ Member-safe View ปกปิด Supplier Identity จนมี Disclosure Grant
* Claim เสนอ Supplier/Logistics-Insurance/Installer ได้ แต่ Order Admin ต้องยืนยันและไม่มี Automatic Compensation
...
ระบบแสดงคำว่า “กลุ่มการผลิต” และแสดงชื่อโรงงานได้เฉพาะเมื่อ Member Profile มี Active Disclosure Grant
ของ Supplier นั้น ห้ามขยายสิทธิ์ตาม Organization และไม่แสดง
...
* Unit Price  
* Line Total  
* Suggested Resale Snapshot — ข้อมูลแนะนำ ไม่รวมยอด  
* Freight Estimate Snapshot — ข้อมูลประมาณการ ไม่รวมยอด  
* Partner Warranty Snapshot  
* Lead Time  
* Validation Status
...
ก่อน Disclosure Grant ให้ Warranty แสดงชื่อ `Partner Warranty` และ Redact Supplier Name/ID/Contact
...
* Previous Claim  
* Delivery Date
* Partner Warranty Snapshot โดยปกปิดชื่อโรงงานตาม Disclosure Rule
...
UI แสดง `ผู้รับผิดชอบที่ระบบแนะนำ` ตาม Issue Type:

* งานผลิต/วัสดุ/สเปก → Supplier Partner  
* ความเสียหายจากขนส่ง → Logistics/Insurance  
* การติดตั้ง → Installer

เป็นคำแนะนำเท่านั้น Order Admin ต้องยืนยันหลังตรวจหลักฐาน
...
* Proposed Resolution  
* Partner Warranty Snapshot  
* Confirmed Responsibility; แสดงชื่อ Supplier เฉพาะเมื่อมี Active Disclosure Grant  
* Timeline  
...
GISP ประสานตามเงื่อนไขของผู้รับผิดชอบต้นทาง ไม่รับรองสิทธิเกิน Partner Warranty และระบบ
ไม่อนุมัติ Warranty หรือคำนวณ Compensation อัตโนมัติ
...
| Confirm Claim Responsibility | No | No | Yes | No | Support | Support | Yes |
| View Supplier Identity | เฉพาะ Active Disclosure Grant ของตน | Existing Grant | Yes | ตาม Permission | ตาม Permission | ตาม Permission | Yes |
...
POST  /api/admin/claims/:id/confirm-responsibility  
...
44. Member ไม่เห็น Factory Cost, Price Formula, Supplier Payment หรือ Margin  
...
51. Order Item เก็บ Suggested Resale/Freight Estimate/Warranty Snapshot โดยไม่ใช้ Suggested Resale รวมยอด  
52. ก่อน Disclosure Grant Order/Claim ไม่แสดง Supplier Name/ID/Contact  
53. Claim เสนอ Responsibility ตาม Issue Type และ Order Admin ยืนยันได้  
54. Supplier/Logistics-Insurance/Installer แยกผู้รับผิดชอบได้  
55. ระบบไม่คำนวณ Compensation หรืออนุมัติ Warranty อัตโนมัติ
...
12. Warranty ต้อง Snapshot และ Claim ต้องระบุผู้รับผิดชอบตามสาเหตุ  
13. ชื่อโรงงานแสดงเฉพาะ Active Disclosure Grant ของ Member Profile  
14. ทุกการเปลี่ยนสถานะสำคัญต้องมีผู้รับผิดชอบและประวัติ
