...
* Core MVP ทำ Supplier, Category, Product, Variant, Option, Media, Document, Factory Cost, Price Formula, Member Price, Suggested Resale, Material Sample/Visit และ Partner Warranty
* Member Price ไม่รวม VAT; Factory Cost/Formula/Margin/Internal Note และ Supplier Identity ก่อน Disclosure เป็นข้อมูลลับ
* Price Formula สืบทอด Global → Supplier → Product และเฉพาะ Super Admin สร้าง แก้ Preview/Activate
* Sample หลักเป็น Material Swatch; เฟอร์นิเจอร์ทั่วไปไม่ใช่ Sample ยกเว้น Built-in Display บางรายการ
...
/admin/catalog/showroom-visits  
/admin/catalog/showroom-visits/\[visitId\]

/admin/catalog/pricing/formulas  
/admin/catalog/pricing/formulas/\[formulaId\]
...
19. Material Sample and Location Management
20. Showroom Visit Operations
...
31. Edit Product  
32. Product Media Manager  
33. Product Document Manager  
34. Product Variant Manager  
35. Product Option Manager  
36. Related Product Manager  
37. Copy Product  
38. Suspend Product  
39. Discontinue Product  
40. Archive Product

## **Pricing**

41. Product Price Overview  
42. Create Factory Cost Version  
43. Create Member Price Version  
44. Price Structure Builder  
45. Price Approval Queue  
46. Price History  
47. Expiring Price List  
48. Bulk Price Update
...
53. Import Preview  
54. Import Result  
55. Import Error Review  
56. Duplicate Review  

## **Publish และ QA**

57. Product Readiness Checklist  
58. Submit for Review  
59. Publish Review Queue  
60. Publish Confirmation  
61. Reject and Return  
62. Catalog QA Dashboard  
63. Missing Data Report  
64. Invalid Price Report  
65. Broken Media Report  
66. Duplicate Product Report  
67. Audit History

รวมประมาณ 67 หน้าจอและสถานะหลัก
...
Member ไม่เห็นชื่อหรือ Contact โรงงานจนกว่า Visit เป็น `COMPLETED` และมี Active Disclosure Grant
ของ Member Profile + Supplier นั้น ห้ามใช้ Organization หรือความสัมพันธ์อื่นขยายสิทธิ์
...
## **23.1 Material Sample and Location Management**

## **Screen ID**

`SMP-001`

จัดการเฉพาะ Sample Type `material_swatch` และ `built_in_display` โดยมี Sample Code,
Option/Material/Built-in Product, รูป, Status, Country, City, Showroom, Shelf และ Note

China Partner Location ต้องแยก `member_display_label` ออกจากชื่อ/ที่อยู่/Contact/Supplier ID ที่เป็น
Confidential และห้ามส่ง Address/Contact/Supplier ID ก่อน Disclosure Grant ทั้ง China Showroom และ Thailand Club
เมื่อ Visit Approved ให้ใช้เฉพาะ Meeting Instruction ที่ไม่เปิดเผยแหล่งสินค้า เฟอร์นิเจอร์ทั่วไปห้ามบันทึกเป็น Sample

## **Visit Operations**

`VISITADM-001` ที่ `/admin/catalog/showroom-visits` แสดงคำขอ `SUBMITTED`, `APPROVED`,
`COMPLETED`, `REJECTED`, `CANCELLED` Purchasing/GISP Admin นัดหมายและใช้ Action Approve/Reject/
Complete การ Complete ต้องสร้าง Disclosure Grant ของ Member Profile + Supplier ใน Transaction เดียว
Super Admin ดูและ Revoke Grant พร้อมเหตุผลได้
...
### **Suggested Resale และ Freight Estimate**

* Suggested Resale Price — แสดงเป็นราคาแนะนำ ไม่ใช่ยอด Order  
* Freight Estimate Low/High — แสดงเป็นประมาณการและ Actual Freight เรียกเก็บแยก  
* Formula Scope/Version — Confidential และเห็นเฉพาะ Super Admin  
* Calculation Snapshot — ห้ามแก้ย้อนหลังเมื่อถูกใช้ใน Order
...
* Material Sample/Built-in Display และ Member-safe Location  
* Factory Name เฉพาะ Active Disclosure Grant  
* Options  
* Documents  
* Remarks  
...
ช่วยตรวจว่า Factory Cost, Formula, Margin, Internal Note หรือ Factory Identity ที่ยังล็อกไม่หลุด
...
* Suggested Resale Version/Amount  
* Freight Estimate Low/High  
* Formula Scope/Version — Super Admin only  
...
# **48\. Price Structure Builder**

## **Screen ID**

`PRICE-004`

เป็นหน้าสร้างสูตรราคาแบบ Versioned และ Preview ผลก่อน Activate เฉพาะ Super Admin เท่านั้น

## **Scope และ Inheritance**

* Scope: Global, Supplier หรือ Product  
* Inherited Formula/Version  
* Effective From/To  
* Status: Draft, Active, Retired  
* ระดับล่าง Override เฉพาะ Component ที่กำหนด ส่วนรายการอื่นสืบทอดจากระดับบน

## **Formula Components**

แต่ละ Row มี Component Code/Name, Calculation Type `percentage` หรือ `fixed_amount_thb`,
Basis `factory_cost_thb` หรือ `member_price`, Value, Include in Member Price และ Inherited/Override Badge

Template เริ่มต้นที่แก้ไขได้:

* Platform Cost 5% ของ Factory Cost THB  
* Marketing/Training/Factory Visit 10% ของ Factory Cost THB  
* Product Sourcing/Catalog Operations 10% ของ Factory Cost THB  
* Suggested Resale Markup 25% ของ Member Price  
* Freight Estimate Low 15% และ High 20% ของ Factory Cost THB แสดงแยก

## **Preview Outputs**

* Factory Cost Source/Currency/Exchange Rate/THB — Confidential  
* Resolved Formula Tree และ Component Breakdown — Confidential  
* Member Price  
* Suggested Resale Price  
* Freight Estimate Low/High  
* Gross Margin/Margin % — Confidential  
* Member-safe Preview

ตัวอย่าง UAT ทุน 100.00 ต้องได้ Member Price 125.00, Suggested Resale 156.25 และ Freight Estimate
15.00–20.00 ใช้ Decimal/Round Half-up สองตำแหน่ง ไม่มี Arbitrary Executable Expression

Actions: `Save Draft`, `Preview`, `Activate`, `Retire` เฉพาะ Super Admin การ Activate สร้าง Audit
และไม่แก้ Price/Order Snapshot เดิมย้อนหลัง
...
* ไม่มี Material Swatch หรือ Built-in Display ที่ Active
...
| ดู Price Formula/Component | No | No | No | No | No | No | Yes |
| สร้าง/แก้/Preview/Activate Formula | No | No | No | No | No | No | Yes |
| จัดการ Material Sample | No | Yes | Yes | Yes | No | Review | Yes |
| Approve/Complete Visit | No | No | View | Yes | No | View | Yes |
| Revoke Supplier Disclosure | No | No | No | No | No | No | Yes |
...
GET    /api/admin/pricing-formulas  
POST   /api/admin/pricing-formulas  
GET    /api/admin/pricing-formulas/:id  
PATCH  /api/admin/pricing-formulas/:id  
POST   /api/admin/pricing-formulas/:id/preview  
POST   /api/admin/pricing-formulas/:id/activate  
POST   /api/admin/pricing-formulas/:id/retire

Formula Endpoints ทั้งหมดใช้ `SUPER_ADMIN` Guard และ Action Endpoint สำหรับ Activate/Retire

## **Material Samples and Visits**

GET    /api/admin/material-samples  
POST   /api/admin/material-samples  
PATCH  /api/admin/material-samples/:id  
GET    /api/admin/showroom-visits  
...
35. Price Formula สืบทอด Global → Supplier → Product และ Override เฉพาะ Component ได้  
36. เฉพาะ Super Admin อ่าน/สร้าง/แก้/Preview/Activate Formula ได้  
37. Formula Template ทุน 100 แสดง Member Price 125, Suggested Resale 156.25 และ Freight 15–20 ได้  
38. Member Preview/API ไม่แสดง Factory Cost, Formula, Margin หรือโรงงานที่ยังล็อก  
39. Suggested Resale/Freight Estimate แสดงเป็นคำแนะนำและไม่รวมยอด Order  
40. Material Sample และ Built-in Display ใช้ Type ถูกต้องและไม่บันทึกเฟอร์นิเจอร์ทั่วไปเป็น Sample  
41. Visit Completed สร้าง Disclosure Grant เฉพาะ Member Profile + Supplier ได้  
42. Super Admin Revoke Disclosure พร้อมเหตุผลและ Audit ได้  
43. Partner Warranty ใช้ Version และ Product/Order Preview แสดงแบบ Redacted ก่อน Disclosure
...
14. Price Approval  
15. Price Structure Builder พร้อม Formula Inheritance/Preview  
16. Material Sample และ Visit Operations  
17. Excel Import Mapping  
...
* Factory Cost  
* Member Price  
* Suggested Resale/Freight Estimate  
* Exchange Rate  
* Price Version  
* Formula Version/Component/Inheritance  
* Formula Preview/Activate — Super Admin only  
* Margin Preview  
* Approval  
* Price Expiry
...
* แสดง Formula, Margin หรือ Supplier Identity ที่ยังไม่ปลดล็อกผ่าน Member API  
* ใช้ Suggested Resale คำนวณยอด Order/Invoice/Payment  
* Activate Formula โดย Role อื่นนอกจาก Super Admin  
...
→ กำหนด Factory Cost  
→ Resolve/Preview Price Structure  
→ กำหนด Member Price และ Suggested Resale  
→ อนุมัติราคา  
→ ตรวจ Catalog QA  
→ Publish  
